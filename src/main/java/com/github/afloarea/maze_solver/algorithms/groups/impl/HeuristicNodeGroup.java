...
public final class HeuristicNodeGroup<T extends GraphNode<T>> implements NodeGroup<T> {
    ...
    public HeuristicNodeGroup(T startNode, T endNode) {
        ...
        queue.add(new Element<>(startNode, null, 0, startNode.getHeuristicTo(endNode)));
    }
    ...
    public void add(Element<T> currentElement, T neighbour, int cost) {
        queue.add(new Element<>(neighbour, currentElement, cost, neighbour.getHeuristicTo(endNode)));
    }
